...
import { startRegistration } from '@simplewebauthn/browser'; // Changé en startRegistration uniquement pour l'enregistrement

const API_BASE = "https://belikanm-kibali-api.hf.space";
const AUTH_API = "https://kibali-iadeploy.onrender.com"; // Serveur Node.js pour l'auth biométrique
...
function App() {
  ...
  // === NOUVEAU : États pour l'authentification biométrique ===
  const [isAuthenticated, setIsAuthenticated] = useState(false);
  const [username] = useState("admin_kibali"); // Tu peux le rendre configurable plus tard

  // === Fonction d'authentification biométrique (enregistrement du dispositif) ===
  const handleBiometricAuth = async () => {
    try {
      // 1. Demander les options d'enregistrement au serveur Node.js
      const optionsRes = await axios.post(`${AUTH_API}/auth/register-options`, { username });
      
      // 2. Déclencher FaceID / TouchID / clé via WebAuthn
      const attestation = await startRegistration(optionsRes.data);
      
      // 3. Envoyer la réponse au serveur pour vérification et stockage dans MongoDB
      const verifyRes = await axios.post(`${AUTH_API}/auth/register-verify`, {
        username,
        body: attestation
      });
      ...
      if (verifyRes.data.verified) {
        ...
        alert("✅ Accès Biométrique validé ! Ton appareil est maintenant lié.");
      }
    } catch (error) {
      console.error("Erreur Auth Biométrique:", error);
      alert("❌ Échec de la biométrie : " + (error.response?.data?.detail || error.message || "Vérifie que ton serveur Node.js est lancé"));
    }
  };
  ...
  return (
    <div style={styles.container}>
      {!isAuthenticated ? (
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#020617' }}>
          <div style={{ padding: '3rem', backgroundColor: '#0f172a', borderRadius: '2rem', border: '1px solid #1e293b', textAlign: 'center' }}>
            <Brain size={60} color="#10b981" style={{ marginBottom: '1.5rem' }} />
            <h2 style={{ fontSize: '1.8rem', color: 'white', marginBottom: '1rem' }}>Accès Sécurisé Kibali</h2>
            <p style={{ color: '#64748b', marginBottom: '2rem' }}>Utilisez FaceID ou votre empreinte pour déverrouiller l'IA.</p>
            <button 
              onClick={handleBiometricAuth}
              style={{ padding: '15px 30px', backgroundColor: '#10b981', color: 'white', border: 'none', borderRadius: '12px', fontWeight: 'bold', cursor: 'pointer' }}
            >
